...
_LANG_MAP = {
    # Full names (match "Malayalam", "Tamil" etc in filenames)
    "malayalam": "Malayalam", "tamil": "Tamil",  "hindi": "Hindi",
    "english":   "English",   "kannada": "Kannada", "telugu": "Telugu",
    "bengali":   "Bengali",   "marathi": "Marathi", "punjabi": "Punjabi",
    # Short codes
    "mal": "Malayalam", "tam": "Tamil",  "hin": "Hindi",
    "eng": "English",   "kan": "Kannada","tel": "Telugu",
    "ben": "Bengali",   "mar": "Marathi","pun": "Punjabi",
}
...
def _build_filter_buttons(key: str, req: int, offset: int,
    ...
    """
    Always render all 4 filter buttons in 2 rows ABOVE the file list.
        Row 1: 🌐 Language ▼   🎬 Quality ▼
        Row 2: 📺 Season ▼     ▶ Episode ▼
    Active selections shown in label. Always visible.
    """
    ...
    def _lbl(prefix, cur):
        return f"{prefix}: {cur} ✅" if cur else f"{prefix} ▼"

    row1 = [
        InlineKeyboardButton(_lbl("🌐 Language", cur_lang),
            callback_data=f"flt_{req}_{key}_L"),
        InlineKeyboardButton(_lbl("🎬 Quality",  cur_qual),
            callback_data=f"flt_{req}_{key}_Q"),
    ]
    row2 = [
        InlineKeyboardButton(_lbl("📺 Season",  cur_season),
            callback_data=f"flt_{req}_{key}_S"),
        InlineKeyboardButton(_lbl("▶ Episode", cur_ep),
...
async def _render_filter_page(
    ...
    if hyper_mode:
        btn = list(filter_rows)
    else:
        btn = list(filter_rows) + _build_file_buttons(page_files, pre, settings)
...
async def give_filter(client, message):
    ...
    # PM search enabled — run auto_filter in both group and PM
...
#ALERT FN IN SPELL CHECK FOR LANGAUGES TO KNOW HOW TO TYPE MOVIES esp english spell check goto adv spell check to check donot change the codes      
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🎬 Browse Movies by Genre', callback_data='genre_menu')
        ],[
