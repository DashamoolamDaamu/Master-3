...
async def admin_statistics(client: Client, query: CallbackQuery):
    ...
    text = (
        ...
        f"🗄 Database: <b>{db_type}</b> — <code>{humanbytes(db_size_bytes)}</code>\n"
        f"🧠 Memory: <code>{ram.percent}% used</code> "
        f"(<code>{humanbytes(ram.used)}</code> / <code>{humanbytes(ram.total)}</code>)\n"
        f"⚡ CPU: <code>{cpu}%</code>\n"
