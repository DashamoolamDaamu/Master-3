...
def _build_genre_menu() -> InlineKeyboardMarkup:
    ...
    for code, name, emoji, _ in GENRES:
        row.append(InlineKeyboardButton(
            f"{emoji} {name}", callback_data=f"g_{code}"))
        if len(row) == 2:
            rows.append(row)
            row = []
    if row:
...
def _build_pagination(uid: int, code: str, offset: int, total: int) -> list:
    ...
    if offset > 0:
        row.append(InlineKeyboardButton(
            "◀ Prev", callback_data=f"gnxt_{uid}_{code}_{max(offset - FILES_PER_PAGE, 0)}"))
    row.append(InlineKeyboardButton(f"📃 {page + 1} / {pages}", callback_data="pages"))
    if offset + FILES_PER_PAGE < total:
        row.append(InlineKeyboardButton(
            "Next ▶", callback_data=f"gnxt_{uid}_{code}_{offset + FILES_PER_PAGE}"))
    return row
...
async def genre_info_page(bot, query):
    ...
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("🎥 Get Movies", callback_data=f"gm_{code}_{uid}")],
        [InlineKeyboardButton("⬅ Back",        callback_data="genre_menu")],
    ])
    await query.answer()
    try:
...
async def get_genre_movies(bot, query):
    ...
    if not files:
        return await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("⬅ Back to Genres", callback_data="genre_menu")]
            ]),
        )
    ...
    if prow:
        btn.append(prow)
    btn.append([InlineKeyboardButton("⬅ Back to Genres", callback_data="genre_menu")])
